...
Texture::create(JNIEnv* env, jobject textureObject)
{
    ...
    // Get data:
    jmethodID texBufferMethodId = env->GetMethodID(textureClass, "getData",
            "()[B");
    ...
    jbyteArray pixelBuffer = (jbyteArray) env->CallObjectMethod(textureObject,
            texBufferMethodId);
    if (pixelBuffer == NULL)
    {
    ...
    newTexture->mData = new unsigned char[newTexture->mWidth
            * newTexture->mHeight * newTexture->mChannelCount];
    ...
    {
        memcpy(newTexture->mData + rowSize * r,
                pixels + rowSize * (newTexture->mHeight - 1 - r),
                newTexture->mWidth * 4);
    }
